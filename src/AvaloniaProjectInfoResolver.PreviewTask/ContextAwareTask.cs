    ...
    public abstract class ContextAwareTask : Task
    {
        ...
        public sealed override bool Execute()
        {
            ...
            var outputPropertiesMap = propertiesMap
                .Where(pair => pair.outerProperty.GetCustomAttribute<OutputAttribute>() != null).ToArray();
        ...
        {
            private readonly ContextAwareTask _loaderTask;

            internal CustomAssemblyLoader(ContextAwareTask loaderTask)
            {
                this._loaderTask = loaderTask;
            }

            protected override Assembly Load(AssemblyName assemblyName)
            {
                var assemblyPath = Path.Combine(_loaderTask.ManagedDllDirectory, assemblyName.Name) + ".dll";
            ...
            protected override IntPtr LoadUnmanagedDll(string unmanagedDllName)
            {
                var unmanagedDllPath = Directory.EnumerateFiles(
                        _loaderTask.UnmanagedDllDirectory,
